// 소모품 카테고리(README "소모품" 항목 참고) — 장착형 장비(GearInstance,
// gear.ts)나 흡수형 정수(essence.ts)와 달리, 들고 다니다가 "사용"하면
// 하나씩 사라지는 아이템. 마석(profile.ts의 ManaStoneCounts)과 같은 카운터
// 방식으로 저장한다 — 같은 종류는 완전히 동일해 개별 인스턴스를 구분할
// 필요가 없다.
//
// 붕대에 이은 두 번째 품목이 포션이다. "포션은 미궁 이동/휴식 중에만 쓰는
// 소모품"으로 확정됐으므로, 전투 화면(ui/battle.ts)이 아니라 미궁 지도
// 화면(ui/dungeon-map.ts)에서만 사용 버튼을 노출한다 — 별도의 "휴식" 화면은
// 아직 없어, 지금 유일하게 전투 밖인 미궁 이동 화면이 그 자리를 대신한다.
// 나머지 후보(수통/식량/담요/나침반/배낭, designnotes.md 2-3번 "탐험가 약탈"
// 참고)는 각각 아직 존재하지 않는 시스템(생존 수치, 인벤토리 확장 등)에
// 연결되어야 비로소 의미가 생기는 아이템들이라 이번 범위 밖.
//
// 정수 해제(README "정수(essence)" 항목 참고)는 이 소모품 카테고리가 아니라
// 신전 시설(ui/temple.ts, profile.ts의 releaseEssence())에서 스톤으로 직접
